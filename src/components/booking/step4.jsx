...
// FIX: Added setIsStepValid to incoming parameters
function Step4({ userInfo, setUserInfo, setIsStepValid }) {
  const [confirmPassword, setConfirmPassword] = useState('');

  const passwordsTyped =
    userInfo.password.length > 0 && confirmPassword.length > 0;
  const passwordMismatch =
    passwordsTyped && userInfo.password !== confirmPassword;
  const passwordTooShort =
    userInfo.password.length > 0 && userInfo.password.length < 8;

  // Run validation whenever any profile info or passwords change
  useEffect(() => {
    const isFormFilled =
    ...
    const arePasswordsValid =
      userInfo.password.length >= 8 && userInfo.password === confirmPassword;

    setIsStepValid(isFormFilled && arePasswordsValid);
  }, [userInfo, confirmPassword, setIsStepValid]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Field label="Phone Number">
            ...
            <input
              ...
              className="flex-1 bg-transparent outline-none text-white text-lg tabular-nums placeholder:text-white/25 placeholder:italic"
            />
          </Field>
        </div>
      </div>
        ...
        {userInfo.password.length >= 8 && !passwordTooShort && (
          <div className="flex items-center gap-2 mt-3 pl-1 animate-[fadeIn_0.2s_ease_both]">
            ...
            <span className="text-xs text-white/40">
              {userInfo.password.length < 11
                ? 'Fair'
                : userInfo.password.length < 14
                  ? 'Good'
                  : 'Strong'}
            </span>
          </div>
