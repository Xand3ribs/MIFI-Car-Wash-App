...
function StepDot({ step, currentStep, label }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <span
        className={`hidden sm:block text-[10px] font-semibold tracking-widest uppercase transition-colors duration-300
...
function StepConnector({ filled }) {
  return (
    <div className="flex-1 h-px mx-1 mt-[-14px] sm:mt-[-18px] relative">
      ...
      <div
        className={`absolute inset-0 bg-blue-action rounded-full transition-all duration-500 origin-left ${filled ? 'scale-x-100' : 'scale-x-0'}`}
      />
    </div>
  );
}
  ...
  return (
    <header className="relative flex-shrink-0 bg-gray-dark border-b border-border-dark">
      <div className="flex items-center h-20 px-5 lg:px-10 gap-6">
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="flex items-center w-full max-w-sm sm:max-w-md">
            {STEP_LABELS.map(({ step, label }, idx) => {
              const adjustedLabel =
                step === 4 && isLoggedIn ? 'Account Secured' : label;
              return (
                <div
                  key={step}
                ...
                >
                  ...
                  {idx < STEP_LABELS.length - 1 && (
                    <StepConnector filled={currentStep > step} />
                  )}
                </div>
              );
            })}
        ...
        <div className="shrink-0 text-right">
          ...
          <p className="text-white font-bold text-lg leading-none">
            {currentStep}
            <span className="text-white/30 font-normal text-sm">/5</span>
          </p>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-white/5">
        <div
          className="h-full bg-blue-action transition-all duration-500 ease-out"
