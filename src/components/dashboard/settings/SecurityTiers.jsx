...
function Field({ label, hasError, children }) {
  return (
    <div className="flex flex-col gap-2">
      ...
      <label
        className={`input flex items-center gap-3 w-full bg-gray-dark rounded-2xl px-4 py-4 border transition-colors duration-200 
    ...
    <>
      <div className="flex flex-col gap-4">
        ...
        >
                ...
                <div className="flex items-center gap-2 mt-2 pl-1 animate-[fadeIn_0.2s_ease_both]">
                  <div className="flex gap-1">
                    {[...Array(4)].map((_, i) => (
                      <div
                        key={i}
                        className={`h-1 w-8 rounded-full transition-colors duration-300 ${pass.new.length >= 8 + i * 3 ? 'bg-green-400' : 'bg-white/10'}`}
                      />
                    ))}
                  </div>
          ...
          {feedback.text && (
            <p
              className={`text-sm font-medium ${feedback.type === 'error' ? 'text-red-400' : 'text-emerald-400'}`}
            >
              {feedback.text}
            </p>
          )}
