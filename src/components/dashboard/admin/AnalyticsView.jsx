...
const MOCK_DB_TRANSACTIONS = [
  { id: 'TXN-001', customer: 'Mike Taylor', car: 'Tesla Model 3', washer: 'Alex Rivera', amount: 25000, status: 'Completed', date: 'Jun 04, 2026', metaMonth: 'June', metaYear: '2026' },
  { id: 'TXN-002', customer: 'Sarah Miller', car: 'Ford F-150', washer: 'Marcus Kruse', amount: 35000, status: 'Completed', date: 'Jun 02, 2026', metaMonth: 'June', metaYear: '2026' },
  { id: 'TXN-003', customer: 'John Doe', car: 'Honda Civic', washer: 'Alex Rivera', amount: 15000, status: 'Completed', date: 'May 14, 2026', metaMonth: 'May', metaYear: '2026' },
  { id: 'TXN-004', customer: 'Emily Davis', car: 'Toyota RAV4', washer: 'Marcus Kruse', amount: 28000, status: 'In Progress', date: 'Jun 08, 2026', metaMonth: 'June', metaYear: '2026' },
  { id: 'TXN-005', customer: 'David Cole', car: 'Lexus RX350', washer: 'Alex Rivera', amount: 40000, status: 'Completed', date: 'Jan 15, 2025', metaMonth: 'January', metaYear: '2025' }
];
...
export default function AnalyticsView() {
  ...
  const filteredMetricsTxns = MOCK_DB_TRANSACTIONS.filter(t => t.metaMonth === metricsMonth && t.metaYear === metricsYear);
  const monthlyRevenue = filteredMetricsTxns.reduce((sum, curr) => curr.status === 'Completed' ? sum + curr.amount : sum, 0);
  const totalBookingsCount = filteredMetricsTxns.length;
  const completedCount = filteredMetricsTxns.filter(t => t.status === 'Completed').length;
  const activeCount = filteredMetricsTxns.filter(t => t.status !== 'Completed').length;
    ...
    <div className="flex-1 h-full overflow-y-auto bg-navy-deep p-6 text-white flex flex-col gap-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Segmented financial accounting matrices</p>
        </div>

        {/* Section 1 Dropdown Filter Group */}
        <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 p-1.5 rounded-xl">
          <span className="text-[10px] text-slate-500 font-bold uppercase px-1">Metrics Scope:</span>
          <select 
            value={metricsMonth} 
