...
export default function WasherOnboardModal({ formData, setFormData, onClose, onSubmit }) {
  return (
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn">
      <div className="bg-gray-dark border border-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-scaleUp">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-800/80 bg-slate-900/50">
          <div>
            <h4 className="font-bold text-white text-base">Onboard New Washer Crew</h4>
            <p className="text-[11px] text-slate-500 mt-0.5">Fill details to provision team system access tokens.</p>
          </div>
          <button 
            type="button" 
            onClick={onClose}
        ...
        <form onSubmit={onSubmit} className="p-5 space-y-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wide">Address Location Base</label>
            <div className="relative">
              <MapPin size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-600" />
              <input 
                type="text"
                required
