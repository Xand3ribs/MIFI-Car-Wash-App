...
import { X, ShieldCheck, XCircle, Car, Clock, MapPin, ArrowRightLeft, FileText } from 'lucide-react';
          ...
          <div>
            ...
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-4">
              <div className="flex flex-col">
                <span className="text-[10px] font-black uppercase tracking-widest text-blue-action">Session Breakdown</span>
                <h3 className="text-xl font-black mt-0.5">Order #{log.id}8294</h3>
              </div>
              <button 
                onClick={onClose}
            ...
            {/* Status Banner */}
            <div className={`mt-5 p-3 rounded-xl flex items-center gap-2 font-bold text-xs uppercase tracking-wider ${
            ...
            <div className="mt-5 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Serviced Vehicle Fleet Profile</p>
                <div className="flex items-center gap-3 bg-navy-deep p-3 rounded-xl border border-slate-800">
                  ...
                  <div>
                    <p className="text-sm font-black text-slate-100">{log.vehicle}</p>
                    <p className="text-[11px] text-slate-400">Premium Sedan Treatment Tier</p>
                  </div>
                </div>
              </div>
              ...
              {/* Time Logs */}
              <div className="flex flex-col gap-1">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Operation Time Stamps</p>
                <div className="grid grid-cols-2 gap-3 bg-navy-deep p-3 rounded-xl border border-slate-800 text-xs">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-slate-500 font-semibold flex items-center gap-1"><Clock size={11} /> Service Started</span>
                    <span className="font-bold text-slate-300">{log.timeStarted || "09:14 AM"}</span>
                  </div>
                  <div className="flex flex-col gap-0.5 border-l border-slate-800 pl-3">
                    <span className="text-slate-500 font-semibold flex items-center gap-1"><Clock size={11} /> Service Completed</span>
                    <span className="font-bold text-slate-300">{log.timeEnded || "10:32 AM"}</span>
                  </div>
                </div>
              </div>
              ...
              {/* Address Location */}
              <div className="flex flex-col gap-1">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Dispatch Address Location</p>
                <div className="flex items-start gap-2 text-xs text-slate-300 bg-navy-deep p-3 rounded-xl border border-slate-800">
                  ...
                  <p className="leading-relaxed font-medium">
                    {log.address || "Plot 14, Admiralty Way, Lekki Phase 1, Lagos"}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                  {role === 'user' ? 'Assigned Detailing Operator' : 'Client Profile Details'}
                </p>
              ...
              <div className="flex flex-col gap-1 mt-1">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Fare Settlement Invoice</p>
                <div className="bg-navy-deep/40 rounded-xl p-3.5 border border-slate-800 text-xs flex flex-col gap-2">
                  <div className="flex justify-between text-slate-400">
                    <span>Base Rate</span>
                    <span className="font-mono">₦{(log.price * 0.85).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-slate-400">
                    <span>Eco Water Levy</span>
                    <span className="font-mono">₦{(log.price * 0.10).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-slate-400">
                    <span>Logistics Surcharge</span>
                    <span className="font-mono">₦{(log.price * 0.05).toLocaleString()}</span>
                  </div>
                  <div className="h-px bg-slate-800 my-1" />
                  <div className="flex justify-between text-sm font-black text-slate-100">
                    <span>Total Net Paid Balance</span>
                    <span className="font-mono text-blue-action">₦{log.price.toLocaleString()}</span>
                  </div>
                </div>
              </div>
