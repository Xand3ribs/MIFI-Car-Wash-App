...
function CancelModal({ isOpen, onClose, onConfirm, isSubmitting }) {
  ...
  const handleSubmit = () => {
    ...
    const finalReason =
      selectedReason === 'Other' ? `Other: ${customNote}` : selectedReason;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-navy-deep border border-slate-800 w-full max-w-md rounded-3xl p-6 relative shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <h3 className="text-xl font-black text-slate-100 pr-6">
          Cancel Appointment
        </h3>
        <p className="text-slate-400 text-xs mt-2 leading-relaxed">
          Are you sure you want to cancel your wash appointment? Please let us
          know why so we can improve our service.
        </p>
